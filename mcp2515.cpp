...
int mcp2515::configRate(int bit_rate)
{
    _mcp2515_printf("\r\nConfig CAB rate = %d",bit_rate);
    int f_osc = 16000000; // 16Mhz
    int tqs; /* tbit/TQ */
    int brp;
    ...
    if (brp >= 8) _mcp2515_printf("Spaztic BRP");
   //     return -1;
    ...
    _mcp2515_printf("\r\nbit rate: BRP = %d, Tbit = %d TQ, PropSeg = %d, PS1 = %d, PS2 = %d, SJW = %d\r\n",
...
uint8_t mcp2515::_spi_readwrite(uint8_t data) {
    //_mcp2515_printf("W0x%x ", data);
    uint8_t ret = spi.write(data);
    //      _mcp2515_printf("R0x%x,", ret);
    return ret;
}
...
void mcp2515::dumpExtendedStatus(void) {
    ...
    _mcp2515_printf("MCP2515 Extended Status:\r\n");
    _mcp2515_printf("MCP Transmit Error Count %d \r\n", tec);
    _mcp2515_printf("MCP Receiver Error Count %d \r\n", rec);
    _mcp2515_printf("MCP Error Flag %d\r\n", eflg);

    if ( (rec>127) || (tec>127) ) {
        _mcp2515_printf("Error-Passive or Bus-Off\r\n");
    }

    if (eflg & MCP_EFLG_RX1OVR)
        _mcp2515_printf("Receive Buffer 1 Overflow\r\n");
    if (eflg & MCP_EFLG_RX0OVR)
        _mcp2515_printf("Receive Buffer 0 Overflow\r\n");
    if (eflg & MCP_EFLG_TXBO)
        _mcp2515_printf("Bus-Off\r\n");
    if (eflg & MCP_EFLG_TXEP)
        _mcp2515_printf("Receive Error Passive\r\n");
    if (eflg & MCP_EFLG_TXWAR)
        _mcp2515_printf("Transmit Error Warning\r\n");
    if (eflg & MCP_EFLG_RXWAR)
        _mcp2515_printf("Receive Error Warning\r\n");
